...
import type { Company } from "./company-data-table"
            ...
            <div className="flex-1 overflow-y-auto">
                <Form {...form}>
                    <form id="edit-company-form" onSubmit={form.handleSubmit(onSubmit)} className="py-2">
                                        ...
                                        >
                                            ...
                                            <SelectContent>
                                                ...
                                                {telemarketers.map((tm) => {
                                                    const initials = tm.full_name?.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2) || tm.email?.[0]?.toUpperCase() || "?"
                                                    return (
                                                        <SelectItem key={tm.id} value={tm.id} className="text-sm">
                                                            <div className="flex items-center gap-2">
                                                                <Avatar className="w-5 h-5 flex-shrink-0 border border-border/50">
                                                                    <AvatarImage src={tm.image_url || undefined} />
                                                                    <AvatarFallback className="text-[10px] bg-muted text-foreground/70">{initials}</AvatarFallback>
                                                                </Avatar>
                                                                {tm.full_name || tm.email?.split("@")[0] || "Unknown"}
                                                            </div>
                                                        </SelectItem>
                                                    )
                                                })}
                        ...
                        {/* Contact & Status - Partially editable by telemarketers */}
                        <CollapsibleSection
